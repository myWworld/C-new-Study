...
public:
	...
	Manager(const Manager& manager) :Employee(manager.name, manager.age, manager.position, manager.rank) { year_of_service = manager.year_of_service; }
	Manager():Employee() {}
...
public:
	...
	void add_employee(Employee* employee)
	{
		...
		{
			alloc_employee = (alloc_employee == 0 ) ? 1 : alloc_employee * 2;
			Employee** prev = employee_list;
			employee_list = new Employee * [alloc_employee];
			
			for (int i = 0; i < current_employee; i++)
			{
				employee_list[i] = prev[i];
			}
...
//	emp_list.add_employee(new Employee("길", 46, "인턴", -2));
//
